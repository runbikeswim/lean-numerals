...
theorem toNat_helper_le_of_le_helper {base : NatGtOne} {a b : List base.Fin}
  (h : le.helper base a b) : toNat.helper base a.toListNat 1 0 ≤ toNat.helper base b.toListNat 1 0 := by
  induction a generalizing b with
  | nil =>
    simp only [List.toListNat_nil_eq, toNat_helper_nil_eq, Nat.zero_le]
  | cons x xs ih =>
    match b with
    | [] =>
      have : equiv.helper base [] (x::xs) := equiv_helper_nil_of_le_helper_nil h
      have : toNat.helper base (x :: xs).toListNat 1 0 = 0 := toNat_helper_eq_zero_of_equiv_helper_nil this
      simp only [this, Nat.zero_le]
    | y::ys =>
      simp only [le_helper_cons_iff] at h
      simp only [List.cons_toListNat_eq, toNat_helper_cons_eq]
      if g : equiv.helper base xs ys then
        simp only [g, reduceIte] at h
        have : toNat.helper base xs.toListNat 1 0 ≤ toNat.helper base ys.toListNat 1 0 :=
            ih (le_helper_of_equiv_helper g)
        calc ↑x + base.val * toNat.helper base xs.toListNat 1 0 ≤ ↑y + base.val * toNat.helper base xs.toListNat 1 0 :=
            Nat.add_le_add_right h (base.val * toNat.helper base xs.toListNat 1 0)
          _ ≤ ↑y + base.val * toNat.helper base ys.toListNat 1 0 :=
            Nat.add_le_add_left (Nat.mul_le_mul_left base.val this) ↑y
      else
        simp only [g, reduceIte] at h
        have h1 : ↑x < base.val := Fin.isLt x
        have h2 : toNat.helper base xs.toListNat 1 0 ≤ toNat.helper base ys.toListNat 1 0 := ih h
        have h3 : toNat.helper base xs.toListNat 1 0 ≠ toNat.helper base ys.toListNat 1 0 := by
          intro hc
          exact absurd (equiv_helper_of_toNat_helper_eq hc) g
        have h4 : toNat.helper base xs.toListNat 1 0 < toNat.helper base ys.toListNat 1 0 := Nat.lt_of_le_of_ne h2 h3
...
theorem le_helper_of_toNat_helper_le {base : NatGtOne} {a b : List base.Fin}
  (h : toNat.helper base a.toListNat 1 0 ≤ toNat.helper base b.toListNat 1 0) : le.helper base a b := by
  induction a generalizing b with
  | nil => exact le_helper_nil
  | cons x xs ih =>
    match b with
    | [] =>
      simp only [List.toListNat_nil_eq, toNat_helper_nil_eq, Nat.le_zero] at h
    ...
    | y::ys =>
      ...
      if g: equiv.helper base xs ys then
        ...
        simp only [g, reduceIte]
        have h1 : toNat.helper base xs.toListNat 1 0 ≠ toNat.helper base ys.toListNat 1 0 := by
          intro hc
          exact absurd (equiv_helper_of_toNat_helper_eq hc) g
        have h2 : toNat.helper base xs.toListNat 1 0 ≤ toNat.helper base ys.toListNat 1 0 :=
...
theorem le_helper_iff_toNat_helper_le {base : NatGtOne} {a b : List base.Fin} :
  le.helper base a b ↔ toNat.helper base a.toListNat 1 0 ≤ toNat.helper base b.toListNat 1 0 :=
...
theorem toNat_helper_lt_toNat_helper_of_lt_helper {base : NatGtOne} {a b : List base.Fin} (h : lt.helper base a b) :
  ...
  have h1 : toNat.helper base a.toListNat  1 0 ≤ toNat.helper base b.toListNat  1 0 :=
...
theorem lt_helper_of_toNat_helper_lt_toNat_helper {base : NatGtOne} {a b : List base.Fin}
  (h : toNat.helper base a.toListNat  1 0 < toNat.helper base b.toListNat  1 0) : lt.helper base a b := by
...
theorem lt_helper_iff_toNat_helper_lt_toNat_helper {base : NatGtOne} {a b : List base.Fin} :
  lt.helper base a b ↔ toNat.helper base a.toListNat 1 0 < toNat.helper base b.toListNat  1 0 :=
