...
theorem equiv_helper_nil_nil {base : NatGtOne} : equiv.helper base [] [] :=
  equiv_helper_nil_iff.mpr (List.all_nil)

theorem equiv_zero_zero {base : NatGtOne} : @zero base ≈ @zero base:=
  equiv_helper_nil_nil
...
theorem toNat_helper_eq_zero_of_equiv_helper_nil {base : NatGtOne} {a : List base.Fin}
  (h: equiv.helper base [] a) : toNat.helper base a.toListNat 1 0 = 0 := by
  induction a with
...
theorem toNat_helper_eq_of_equiv_helper {base : NatGtOne} {a b : List base.Fin} (h: equiv.helper base a b) :
  ...
  induction a generalizing b with
  | nil => simp only [List.toListNat_nil_eq, toNat_helper_eq_zero_of_equiv_helper_nil h, toNat_helper_nil_eq]
  | cons x xs ih =>
    match b with
    | [] => simp only [List.toListNat_nil_eq, toNat_helper_eq_zero_of_equiv_helper_nil (equiv_helper_symm h), toNat_helper_nil_eq]
    | y::ys =>
...
theorem equiv_helper_nil_of_toNat_helper_zero {base : NatGtOne} {a : List base.Fin}
  ...
  | cons x xs ih =>
    ...
    have h2 : base.val = 0 ∨ toNat.helper base xs.toListNat 1 0 = 0 :=
      Nat.zero_eq_mul.mp (Eq.symm (Nat.eq_zero_of_add_eq_zero_left h))
    have h3 : toNat.helper base xs.toListNat 1 0 = 0 := by
      cases h2 with
...
theorem equiv_helper_of_toNat_helper_eq {base : NatGtOne} {a b : List base.Fin}
  ...
  | cons x xs ih =>
    ...
    | y::ys =>
      ...
      have : x.val = y.val ∧ toNat.helper base xs.toListNat 1 0 = toNat.helper base ys.toListNat 1 0 :=
