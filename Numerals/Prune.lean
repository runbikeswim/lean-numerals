...
def prune {base : NatGtOne} (a : List Nat) (n := 0) : TZNumeral base where
...
theorem prune_helper_toListAux_eq {base : NatGtOne} {a : List base.Fin} :
  prune.helper base a.toListNat 0 = a := by
  induction a with
  | nil => simp only [List.toListNat_nil_eq, prune_helper_nil_zero_eq_nil]
  | cons x xs ih =>
    have h1 : ↑x / base.val = 0 := Nat.div_eq_zero_iff.mpr (.inr (Fin.is_lt x))
    have h2 : FinBase.ofNat ↑x = x := Fin.ofNat_val_eq_self x
    simp only [List.cons_toListNat_eq, prune_helper_cons_eq, Nat.add_zero, h1, ih, h2]
...
theorem prune_helper_nil_noTrailingZero {base : NatGtOne} {n : Nat} :
  (prune.helper base [] n).noTrailingZero := by
  induction n using Nat.strongRecOn with
  | _ l ihl =>
    match gl : l with
    | 0 => simp only [prune_helper_nil_zero_eq_nil]; exact List.nil_noTrailingZero
    | k + 1 =>
      simp only [prune.helper]
      ...
      if g : (k + 1) / base.val = 0 then
        ...
        have h4 : List.noTrailingZero (prune.helper base [] ((k + 1) / base.val))
        ...
        exact List.cons_noTrailingZero_of h4
      else
...
theorem prune_helper_noTrailingZero_of_noTrailingZero {base : NatGtOne} {n : Nat} {a : List Nat} (h : a.noTrailingZero) :
  ...
  | cons x xs iha =>
    ...
    simp only [List.cons_noTrailingZero_iff_noTrailingZero_and, h2, true_and]
    intro h
...
theorem toNat_helper_prune_helper_nil_eq {base : NatGtOne} {n : Nat} :
  toNat.helper base (prune.helper base [] n).toListNat 1 0 = n := by
  induction n using Nat.strongRecOn with
  | _ l ih =>
    match gl : l with
    | 0 => simp only [prune_helper_nil_zero_eq_nil, List.toListNat, List.mapCoe, List.map_nil, toNat.helper]
    | k + 1 =>
      have : (k + 1) / base.val < k + 1 := Nat.div_lt_self (Nat.succ_pos k) base.property
      simp only [List.toListNat, List.mapCoe, CoeOut.coe] at ih
      simp only [prune.helper, List.toListNat, List.mapCoe, List.map_cons, toNat_helper_cons_eq, FinBase.ofNat, CoeOut.coe]
      simp only [ih ((k + 1) / base.val) this]
      rw [Nat.add_comm]
      exact Nat.div_add_mod (k + 1) base.val

theorem toNat_helper_prune_helper_toListNat_eq {base : NatGtOne} {a : List Nat} {n : Nat} :
  toNat.helper base (prune.helper base a n).toListNat 1 0
    = n + (toNat.helper base (prune.helper base a 0).toListNat 1 0) := by
  induction a generalizing n with
  | nil => simp only [toNat_helper_prune_helper_nil_eq, Nat.add_zero]
  | cons x xs ih =>
    simp only [prune_helper_cons_eq, List.toListNat, List.mapCoe, CoeOut.coe, FinBase.ofNat] at ⊢ ih
    ...
    simp only [Fin.toNat, Nat.mod_add_div (x + n) base.val, Nat.mod_add_div x base.val, ← Nat.add_assoc]
    rw (occs := .pos [2]) [Nat.add_comm]

theorem toNat_helper_prune_helper_mapCoe_eq {base : NatGtOne} {a : List Nat} {n : Nat} :
  toNat.helper base (prune.helper base a n).mapCoe 1 0
    = n + (toNat.helper base (prune.helper base a 0).mapCoe 1 0) :=
  toNat_helper_prune_helper_toListNat_eq
...
theorem toNat_prune_eq_add_toNat_prune_zero {base : NatGtOne} {a : TZNumeral base} {n : Nat}  :
  ...
  exact toNat_helper_prune_helper_toListNat_eq
