...
theorem tonz_noTrailingZero_of {base : NatGtOne} {n : base.Fin} {a : List base.Fin}
  (ha :a.noTrailingZero) : (tonz n a).noTrailingZero := by
  unfold tonz
  match a with
  | [] =>
    if g : n = 0 then
  ...
  | x::xs =>
    ...
    exact List.cons_noTrailingZero_of (And.intro ha this)
...
theorem discardTZ_helper_noTrailingZero {base : NatGtOne} {l : List base.Fin} :
  (discardTZ.helper base l).noTrailingZero := by
  induction l with
  | nil => simp only [discardTZ_helper_nil_eq_nil]; exact List.nil_noTrailingZero
  | cons x xy ih =>
    simp only [discardTZ.helper]
    exact tonz_noTrailingZero_of ih
...
theorem discardTZ_noTrailingZero {base : NatGtOne} {n : TZNumeral base} :
  ...
  exact discardTZ_helper_noTrailingZero
